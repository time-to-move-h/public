...
$IDMAQ = trim(isset($_GET['EIDMAQ']) ? $_GET['EIDMAQ'] : '');
...
// Suppression
function supprimerSousmaquettes($IDAPP,$IDCHP,$IDMAQ) {
	global $data;
	return $data->executeQuery("DELETE FROM sousmaquettes WHERE IDAPP = '$IDAPP' AND IDCHP = '$IDCHP' AND IDMAQ = '$IDMAQ';");
}
...
if (isset($_POST["BSupprimer"])) {
	...
	if (isset($_POST["ID"])) {
	    ...
	    for ($i = 0; $i < count($id); $i++) {
			$tab = preg_split("/[#]+/", $id[$i]);
			if (count($tab > 0)) {
					supprimerSousmaquettes(addslashes($tab[0]),addslashes($tab[1]),addslashes($tab[2]));					
			}
		}
	}
}
...
if (isset($_POST["BModifier"])) {
	if (isset($_POST["ID"])) {
		...
		header("Location: FrmEdit_sousmaquettes.php?" . "EIDAPP=" . addslashes($tab[0]) . '&' . "EIDCHP=" . addslashes($tab[1]) . '&' . "EIDMAQ=" . addslashes($tab[2]));									
		}	
	}
}
...
if (isset($_POST["BCreer"])) {		
	header("Location: FrmEdit_sousmaquettes.php");				
}

// ########################################
// Accès Data
$strSql = "Select * From sousmaquettes Where IDAPP='$IDAPP' AND IDMAQ='$IDMAQ'";
...
<form method="post" action="<?php echo $page;?>" name="FSousmaquettes">
...
<table width="100%"  border="1">
  ...
  <tr>
    <td colspan="14"><?php echo $NbreEnreg;?> Sousmaquettes</td>
  </tr>
  <tr>
    <td width="6%">&nbsp;</td>
		<td width="15%"></td>
  ...
  <?php
  	while ($enreg = $util->getLineObject($res)) {  
  		$tab = "  <tr>";
    	$tab .= " <td>";  	
		$tab .= "<input name=\"ID[]\" type=\"checkbox\" id=\"ID[]\" value=\"" . $enreg->IDAPP . "#" . $enreg->IDCHP . "#" . $enreg->IDMAQ . "\"/>";
		
		$tab .= "</td>";
		$tab .= "<td>" . $enreg->IDAPP . "</td>";
		$tab .= "<td>" . $enreg->IDMAQ . "</td>";
  ...
  ?>
  <tr>
    <td colspan="14"><input name="BCreer" type="submit" id="BCreer" value="Creer">
...
if ($NbreEnreg > $EnregMax) {
	...
	// Transmission Data
	$var_pri = "&EIDAPP=$IDAPP&EIDCHP=$IDCHP&EIDMAQ=$IDMAQ";		
