...
// Suppression
function supprimerApps($IDAPP) {
	global $data;
	return $data->executeQuery("DELETE FROM apps WHERE IDAPP = '$IDAPP';");
}
...
if (isset($_POST["BSupprimer"])) {
	if (isset($_POST["ID"])) {
	    ...
	    for ($i = 0; $i < count($id); $i++) {
			$tab = preg_split("/[#]+/", $id[$i]);
			if (count($tab > 0)) {
					supprimerApps(addslashes($tab[0]));					
			}
		}
	}
}
...
if (isset($_POST["BModifier"])) {
	if (isset($_POST["ID"])) {
		...
		if (count($id) > 0) {
			...
			header("Location: FrmEdit_apps.php?" . "EIDAPP=" . addslashes($tab[0]));									
		}	
	}
}
...
if (isset($_POST["BCreer"])) {		
    header("Location: FrmEdit_apps.php");				
}

// ########## LINK LISTE #########

if (isset($_POST["BSubList"])) {
	if (isset($_POST["ID"])) {
	$id = $_POST["ID"];
 	if (count($id) > 0) {
		$data->disconnect();
	...
	header("Location: FrmView_apps_data.php?" . "EIDAPP=" . addslashes($tab[0]));
}}}


// ########################################
// Accès Data
$strSql = "Select * From apps";
...
<form method="post" action="<?php echo $page;?>" name="F1">
...
<table width="100%"  border="1">
  ...
  <tr>
    <td colspan="4"><?php echo $NbreEnreg;?> Apps</td>
  </tr>
  <tr>
    <td width="6%">&nbsp;</td>
		<td width="15%"></td>
  ...
  <?php
  	while ($enreg = $data->fetchObject()) {  
  		$tab = "  <tr>";
    	$tab .= " <td>";  	
		$tab .= "<input name=\"ID[]\" type=\"checkbox\" id=\"ID[]\" value=\"" . $enreg->IDAPP . "\"/>";
  ...
  <tr>
    <td colspan="4"><input name="BCreer" type="submit" id="BCreer" value="Creer">
      <input name="BSupprimer" type="submit" id="BSupprimer" value="Supprimer">
        ...
        <input name="BSubList" id="BSubList" type="submit" value="Templates">
