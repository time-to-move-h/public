...
function supprimerApps_subdata($IDAPP,$IDAPPDAT,$IDCHP) {
	...
	try {
		$strSql = "DELETE FROM apps_subdata WHERE IDAPP = '$IDAPP' AND IDAPPDAT = '$IDAPPDAT' AND IDCHP = '$IDCHP' LIMIT 1;";
		//exit(var_dump($strSql));
		$result = $data->executeQuery($strSql);
	} catch (\Throwable $th) {
		error_log("RAD : supprimerApps_subdata");
	}
...
// ########################################
// Accès Data
$strSql = "Select * From apps_subdata Where IDAPP='$IDAPP' AND IDAPPDAT=$IDAPPDAT ORDER BY NUMSEQ";
...
<form method="post" action="<?php echo $page;?>" name="F7">
