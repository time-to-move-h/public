...
// Ajout
function ajouterSouslistes($IDAPP,$IDLST,$IDCHP,$LIBELLE,$DESCRIPTION,$REQUETE,$NUMSEQ,$TYPEDONNEE,$PRIMAIRE,$VISIBLE,$TAILLE,$AI,$NUL) {
	global $data;			
	...
	$strSql = "INSERT INTO souslistes (AI, DESCRIPTION, IDAPP, IDCHP, IDLST, LIBELLE, NUL, NUMSEQ, PRIMAIRE, REQUETE, TAILLE, TYPEDONNEE, VISIBLE) VALUES (";
...
// Modification
function modifierSouslistes($AI, $DESCRIPTION, $IDAPP, $IDCHP, $IDLST, $LIBELLE, $NUL, $NUMSEQ, $PRIMAIRE, $REQUETE, $TAILLE, $TYPEDONNEE, $VISIBLE) {
	global $data, $util, $AI_ori, $DESCRIPTION_ori, $IDAPP_ori, $IDCHP_ori, $IDLST_ori, $LIBELLE_ori, $NUL_ori, $NUMSEQ_ori, $PRIMAIRE_ori, $REQUETE_ori, $TAILLE_ori, $TYPEDONNEE_ori, $VISIBLE_ori;
	$strSql = 'UPDATE souslistes SET ';
	$strSql_Req = '';		

	if (testSouslistes($IDAPP_ori, $IDCHP_ori, $IDLST_ori)) {			
				
	$result = getSingleRowSouslistes(	$IDAPP_ori, 	$IDCHP_ori, 	$IDLST_ori);
	...
	$strSql .= $strSql_Req . " WHERE IDAPP = '$IDAPP_ori' AND IDCHP = '$IDCHP_ori' AND IDLST = '$IDLST_ori'";	
	//echo $strSql;
	return $data->executeQuery($strSql);
}

// Test d'existence
function testSouslistes($IDAPP, $IDCHP, $IDLST) {
	global $data;
	$strSql = "SELECT 1 FROM souslistes WHERE  IDAPP = '$IDAPP' AND IDCHP = '$IDCHP' AND IDLST = '$IDLST' ;";
	$res = $data->executeQuery($strSql);
	return $data->testCommun($res);
}

// Récupération enregistrement
function getSingleRowSouslistes($IDAPP, $IDCHP, $IDLST) {
	global $data;
	$strSql = "SELECT * FROM souslistes WHERE IDAPP = '$IDAPP' AND IDCHP = '$IDCHP' AND IDLST = '$IDLST';";
	return $data->executeQuery($strSql);
}
...
switch ($mode) {
	case 0 :	
if (! testSouslistes($IDAPP, $IDCHP, $IDLST)) {
					
if (ajouterSouslistes($IDAPP,$IDLST,$IDCHP,$LIBELLE,$DESCRIPTION,$REQUETE,$NUMSEQ,$TYPEDONNEE,$PRIMAIRE,$VISIBLE,$TAILLE,$AI,$NUL)) {
...
if (modifierSouslistes($AI, $DESCRIPTION, $IDAPP, $IDCHP, $IDLST, $LIBELLE, $NUL, $NUMSEQ, $PRIMAIRE, $REQUETE, $TAILLE, $TYPEDONNEE, $VISIBLE)) {				
...
// ----------------------------------- LECTURE
if (isset($_GET["EIDAPP"]) && isset($_GET["EIDCHP"]) && isset($_GET["EIDLST"])) {
...
$result = getSingleRowSouslistes($IDAPP, $IDCHP, $IDLST);
$ligne = $util->getFirstLineObject($result);	
// Récupération d'un tableau d'enregistrement	
if (count($ligne) > 0) {
	$AI = $AI_ori = $ligne->AI;
...
<form action="<?php echo $page; ?>" method="post" name="FSouslistes" id="FSouslistes">
<div class="CSSTableGenerator">
<table width="500" border="1" align="center">  
        <tr>
          <td height="12" colspan="2"><div align="center">Souslistes</div></td>
          </tr>
        <tr>
<td></td>
...
<td><input type="checkbox" name="ENUL"  id="ENUL" <?php if ($NUL == "1")  echo "checked=\"checked\""; ?>></td>
      ...
      <td>&nbsp;</td>
		...
		<input id="EIDAPP_ori" name="EIDAPP_ori" type="hidden"  value="<?php echo $IDAPP_ori; ?>"><input id="EIDCHP_ori" name="EIDCHP_ori" type="hidden"  value="<?php echo $IDCHP_ori; ?>"><input id="EIDLST_ori" name="EIDLST_ori" type="hidden"  value="<?php echo $IDLST_ori; ?>">
