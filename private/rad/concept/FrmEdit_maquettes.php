...
// Ajout
function ajouterMaquettes($IDAPP,$IDMAQ,$LIBELLE,$DESCRIPTION,$REQUETE,$IDENG) {
	global $data;			
	...
	$strSql = "INSERT INTO maquettes (DESCRIPTION, IDAPP, IDENG, IDMAQ, LIBELLE, REQUETE) VALUES (";
...
// Modification
function modifierMaquettes($DESCRIPTION, $IDAPP, $IDENG, $IDMAQ, $LIBELLE, $REQUETE) {
	global $data, $util, $DESCRIPTION_ori, $IDAPP_ori, $IDENG_ori, $IDMAQ_ori, $LIBELLE_ori, $REQUETE_ori;
	$strSql = 'UPDATE maquettes SET ';
	$strSql_Req = '';		

	if (testMaquettes($IDAPP_ori, $IDMAQ_ori)) {			
				
	$result = getSingleRowMaquettes(	$IDAPP_ori, 	$IDMAQ_ori);
	...
	$strSql .= $strSql_Req . " WHERE IDAPP = '$IDAPP_ori' AND IDMAQ = '$IDMAQ_ori'";	
	//echo $strSql;
	return $data->executeQuery($strSql);
}

// Test d'existence
function testMaquettes($IDAPP, $IDMAQ) {
	global $data;
	$strSql = "SELECT 1 FROM maquettes WHERE  IDAPP = '$IDAPP' AND IDMAQ = '$IDMAQ' ;";
	$res = $data->executeQuery($strSql);
	return $data->testCommun($res);
}

// Récupération enregistrement
function getSingleRowMaquettes($IDAPP, $IDMAQ) {
	global $data;
	$strSql = "SELECT * FROM maquettes WHERE IDAPP = '$IDAPP' AND IDMAQ = '$IDMAQ';";
	return $data->executeQuery($strSql);
}
...
switch ($mode) {
	case 0 :	
if (! testMaquettes($IDAPP, $IDMAQ)) {
					
if (ajouterMaquettes($IDAPP,$IDMAQ,$LIBELLE,$DESCRIPTION,$REQUETE,$IDENG)) {
...
if (modifierMaquettes($DESCRIPTION, $IDAPP, $IDENG, $IDMAQ, $LIBELLE, $REQUETE)) {				
...
// ----------------------------------- LECTURE
if (isset($_GET["EIDAPP"]) && isset($_GET["EIDMAQ"])) {
...
$result = getSingleRowMaquettes($IDAPP, $IDMAQ);
$ligne = $util->getFirstLineObject($result);	
// Récupération d'un tableau d'enregistrement	
if (count($ligne) > 0) {
...
<table width="500" border="1" align="center">  
        <tr>
          <td height="12" colspan="2"><div align="center">Maquettes</div></td>
          </tr>
        <tr>
<td>ID APP</td>
...
<td><input type="text" size="30" name="EIDENG" id="EIDENG" value="<?php echo htmlentities($IDENG, ENT_COMPAT,'UTF-8', true); ?>"></td>
      ...
      <td>&nbsp;</td>
		...
		<input id="EIDAPP_ori" name="EIDAPP_ori" type="hidden"  value="<?php echo $IDAPP_ori; ?>"><input id="EIDMAQ_ori" name="EIDMAQ_ori" type="hidden"  value="<?php echo $IDMAQ_ori; ?>">
