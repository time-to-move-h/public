...
// Ajout
function ajouterSousmaquettes($IDAPP,$IDMAQ,$IDCHP,$LIBELLE,$DESCRIPTION,$REQUETE,$NUMSEQ,$TYPEDONNEE,$PRIMAIRE,$VISIBLE,$TAILLE,$AI,$NUL) {
	global $data;			
		
	$IDAPP = addslashes($IDAPP);
	$IDMAQ = addslashes($IDMAQ);
	...
	$strSql = "INSERT INTO sousmaquettes (AI, DESCRIPTION, IDAPP, IDCHP, IDMAQ, LIBELLE, NUL, NUMSEQ, PRIMAIRE, REQUETE, TAILLE, TYPEDONNEE, VISIBLE) VALUES (";
...
// Modification
function modifierSousmaquettes($AI, $DESCRIPTION, $IDAPP, $IDCHP, $IDMAQ, $LIBELLE, $NUL, $NUMSEQ, $PRIMAIRE, $REQUETE, $TAILLE, $TYPEDONNEE, $VISIBLE) {
	global $data, $util, $AI_ori, $DESCRIPTION_ori, $IDAPP_ori, $IDCHP_ori, $IDMAQ_ori, $LIBELLE_ori, $NUL_ori, $NUMSEQ_ori, $PRIMAIRE_ori, $REQUETE_ori, $TAILLE_ori, $TYPEDONNEE_ori, $VISIBLE_ori;
	$strSql = 'UPDATE sousmaquettes SET ';
	$strSql_Req = '';		

	if (testSousmaquettes($IDAPP_ori, $IDCHP_ori, $IDMAQ_ori)) {			
				
	$result = getSingleRowSousmaquettes(	$IDAPP_ori, 	$IDCHP_ori, 	$IDMAQ_ori);
	...
	if ($IDMAQ != addslashes($IDMAQ_ori)) $strSql_Req .= ((strlen($strSql_Req) > 0) ? " , " : "") . "IDMAQ = '$IDMAQ'";
	...
	$strSql .= $strSql_Req . " WHERE IDAPP = '$IDAPP_ori' AND IDCHP = '$IDCHP_ori' AND IDMAQ = '$IDMAQ_ori'";	
	//echo $strSql;
	return $data->executeQuery($strSql);
}

// Test d'existence
function testSousmaquettes($IDAPP, $IDCHP, $IDMAQ) {
	global $data;
	$strSql = "SELECT 1 FROM sousmaquettes WHERE  IDAPP = '$IDAPP' AND IDCHP = '$IDCHP' AND IDMAQ = '$IDMAQ' ;";
	$res = $data->executeQuery($strSql);
	return $data->testCommun($res);
}

// Récupération enregistrement
function getSingleRowSousmaquettes($IDAPP, $IDCHP, $IDMAQ) {
	global $data;
	$strSql = "SELECT * FROM sousmaquettes WHERE IDAPP = '$IDAPP' AND IDCHP = '$IDCHP' AND IDMAQ = '$IDMAQ';";
	return $data->executeQuery($strSql);
}
...
$IDMAQ = trim(isset($_POST['EIDMAQ']) ? $_POST['EIDMAQ'] : '');
...
$IDMAQ_ori = trim(isset($_POST['EIDMAQ_ori']) ? $_POST['EIDMAQ_ori'] : '');
	
	
$mode = $_POST["HMode"];
switch ($mode) {
	case 0 :	
if (! testSousmaquettes($IDAPP, $IDCHP, $IDMAQ)) {
					
if (ajouterSousmaquettes($IDAPP,$IDMAQ,$IDCHP,$LIBELLE,$DESCRIPTION,$REQUETE,$NUMSEQ,$TYPEDONNEE,$PRIMAIRE,$VISIBLE,$TAILLE,$AI,$NUL)) {
...
if (modifierSousmaquettes($AI, $DESCRIPTION, $IDAPP, $IDCHP, $IDMAQ, $LIBELLE, $NUL, $NUMSEQ, $PRIMAIRE, $REQUETE, $TAILLE, $TYPEDONNEE, $VISIBLE)) {				
...
// ----------------------------------- LECTURE
if (isset($_GET["EIDAPP"]) && isset($_GET["EIDCHP"]) && isset($_GET["EIDMAQ"])) {
...
$result = getSingleRowSousmaquettes($IDAPP, $IDCHP, $IDMAQ);
$ligne = $util->getFirstLineObject($result);	
// Récupération d'un tableau d'enregistrement	
if (count($ligne) > 0) {
	$AI = $AI_ori = $ligne->AI;
...
<form action="<?php echo $page; ?>" method="post" name="FSousmaquettes" id="FSousmaquettes">
<div class="CSSTableGenerator">
<table width="500" border="1" align="center">  
        <tr>
          <td height="12" colspan="2"><div align="center">Sousmaquettes</div></td>
          </tr>
        <tr>
<td>ID App</td>
...
<tr>
<td>ID Maq</td>
<td><input type="text" size="30" name="EIDMAQ" id="EIDMAQ" value="<?php echo htmlentities($IDMAQ, ENT_COMPAT,'UTF-8', true); ?>"></td>
</tr>
<tr>
<td>ID Champ</td>
...
<td><input type="checkbox" name="ENUL"  id="ENUL" <?php if ($NUL == "1")  echo "checked=\"checked\""; ?>></td>
      ...
      <td>&nbsp;</td>
		...
		<input id="EIDAPP_ori" name="EIDAPP_ori" type="hidden"  value="<?php echo $IDAPP_ori; ?>"><input id="EIDCHP_ori" name="EIDCHP_ori" type="hidden"  value="<?php echo $IDCHP_ori; ?>"><input id="EIDMAQ_ori" name="EIDMAQ_ori" type="hidden"  value="<?php echo $IDMAQ_ori; ?>">
