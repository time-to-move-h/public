...
// Suppression
function supprimerSouslistes($IDAPP,$IDCHP,$IDLST) {
	global $data;
	return $data->executeQuery("DELETE FROM souslistes WHERE IDAPP = '$IDAPP' AND IDCHP = '$IDCHP' AND IDLST = '$IDLST';");
}
...
if (isset($_POST["BSupprimer"])) {
	...
	if (isset($_POST["ID"])) {
	    ...
	    for ($i = 0; $i < count($id); $i++) {
			$tab = preg_split("/[#]+/", $id[$i]);
			if (count($tab > 0)) {
					supprimerSouslistes(addslashes($tab[0]),addslashes($tab[1]),addslashes($tab[2]));					
			}
		}
	}
}
...
if (isset($_POST["BModifier"])) {
	if (isset($_POST["ID"])) {
		...
		header("Location: FrmEdit_souslistes.php?" . "EIDAPP=" . addslashes($tab[0]) . '&' . "EIDCHP=" . addslashes($tab[1]) . '&' . "EIDLST=" . addslashes($tab[2]));									
		}	
	}
}
...
if (isset($_POST["BCreer"])) {		
	header("Location: FrmEdit_souslistes.php");				
}

// ########################################
// Accès Data
$strSql = "Select * From souslistes Where IDAPP='$IDAPP' AND IDLST='$IDLST'";
...
<form method="post" action="<?php echo $page;?>" name="FSouslistes">
...
<table width="100%"  border="1">
  ...
  <tr>
    <td colspan="14"><?php echo $NbreEnreg;?> Souslistes</td>
  </tr>
  <tr>
    <td width="6%">&nbsp;</td>
		<td width="15%"></td>
  ...
  <?php
  	while ($enreg = $util->getLineObject($res)) {  
  		$tab = "  <tr>";
    	$tab .= " <td>";  	
		$tab .= "<input name=\"ID[]\" type=\"checkbox\" id=\"ID[]\" value=\"" . $enreg->IDAPP . "#" . $enreg->IDCHP . "#" . $enreg->IDLST . "\"/>";
...
if ($NbreEnreg > $EnregMax) {
	...
	// Transmission Data
	$var_pri = "&EIDAPP=$IDAPP&EIDCHP=$IDCHP&EIDLST=$IDLST";		
