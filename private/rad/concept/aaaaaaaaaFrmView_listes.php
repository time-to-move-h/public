...
// Suppression
function supprimerListes($IDAPP,$IDLST) {
	global $data;
	return $data->executeQuery("DELETE FROM listes WHERE IDAPP = '$IDAPP' AND IDLST = '$IDLST';");
}
...
if (isset($_POST["BSupprimer"])) {
	if (isset($_POST["ID"])) {
	    ...
	    for ($i = 0; $i < count($id); $i++) {
			$tab = preg_split("/[#]+/", $id[$i]);
			if (count($tab > 0)) {
					supprimerListes(addslashes($tab[0]),addslashes($tab[1]));					
			}
		}
	}
}
...
if (isset($_POST["BModifier"])) {
	if (isset($_POST["ID"])) {
		...
		if (count($id > 0) ) {
			...
			header("Location: FrmEdit_listes.php?" . "EIDAPP=" . addslashes($tab[0]) . '&' . "EIDLST=" . addslashes($tab[1]));									
		}	
	}
}
...
if (isset($_POST["BCreer"])) {		
	header("Location: FrmEdit_listes.php");				
}
...
if (isset($_POST["BSubImport"])) {
	...
	header("Location: FrmImport_listes.php?" . "EIDAPP=" . addslashes($tab[0]) . '&' . "EIDLST=" . addslashes($tab[1]));
}}}
if (isset($_POST["BSubChamps"])) {
	...
	header("Location: FrmView_souslistes.php?" . "EIDAPP=" . addslashes($tab[0]) . '&' . "EIDLST=" . addslashes($tab[1]));
}}}
...
if (isset($_POST["BSubLinkList"])) {
	...
	header("Location: FrmView_lnklist.php?" . "EIDAPP=" . addslashes($tab[0]) . '&' . "EIDLST=" . addslashes($tab[1]));
}}}


// ########################################
// Accès Data
$strSql = "Select * From listes";
...
<form method="post" action="<?php echo $page;?>" name="FListes">
...
<table width="100%"  border="1">
  ...
  <tr>
    <td colspan="7"><?php echo $NbreEnreg;?> Listes</td>
  </tr>
  <tr>
    <td width="6%">&nbsp;</td>
		<td width="15%"></td>
  ...
  <?php
  	while ($enreg = $util->getLineObject($res)) {  
  		$tab = "  <tr>";
    	$tab .= " <td>";  	
		$tab .= "<input name=\"ID[]\" type=\"checkbox\" id=\"ID[]\" value=\"" . $enreg->IDAPP . "#" . $enreg->IDLST . "\"/>";
...
if ($NbreEnreg > $EnregMax) {
	...
	// Transmission Data
	$var_pri = "&EIDAPP=$IDAPP&EIDLST=$IDLST";		
