...
$IDENG = trim(isset($_GET['EIDENG']) ? $_GET['EIDENG'] : '');
...
// Suppression
function supprimerEngine_config($IDENG) {
    global $data;
    return $data->executeQuery("DELETE FROM engine_config WHERE IDENG = $IDENG;");
}
...
if (isset($_POST["BSupprimer"])) {
	...
	if (isset($_POST["ID"])) {
	    ...
	    for ($i = 0; $i < count($id); $i++) {
			$tab = preg_split("/[#]+/", $id[$i]);
			if (count($tab > 0)) {
					supprimerEngine_config(addslashes($tab[0]));					
			}
		}
	}
}
...
if (isset($_POST["BModifier"])) {
    if (isset($_POST["ID"])) {
        ...
        header("Location: FrmEdit_engine_config.php?" . "EIDENG=" . addslashes($tab[0]));									
        }	
    }
}
...
if (isset($_POST["BCreer"])) {		
    header("Location: FrmEdit_engine_config.php");				
}

// ########################################
// Accès Data
$strSql = "Select * From engine_config";
...
<form method="post" action="<?php echo $page;?>" name="FEngine_config">
...
<table width="100%"  border="1">
  ...
  <tr>
    <td colspan="4"><?php echo $NbreEnreg;?> Engine_config</td>
  </tr>
  <tr>
    <td width="6%">&nbsp;</td>
		<td width="15%">ID Engine</td>
	<td width="15%">Fichier Source</td>
	<td width="15%">Fichier Destination</td>
</tr>
  ...
  <?php
  	while ($enreg = $data->fetchObject()) {  
  		$tab = "  <tr>";
    	$tab .= " <td>";  	
		$tab .= "<input name=\"ID[]\" type=\"checkbox\" id=\"ID[]\" value=\"" . $enreg->IDENG . "\"/>";
		
		$tab .= "</td>";
		$tab .= "<td>" . $enreg->IDENG . "</td>";
		$tab .= "<td>" . $enreg->FILESOU . "</td>";
		$tab .= "<td>" . $enreg->FILEDES . "</td>";
  ...
  ?>
  <tr>
    <td colspan="4"><input name="BCreer" type="submit" id="BCreer" value="Creer">
