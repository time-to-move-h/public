...
if (isset($_POST["BCreer"])) {		
	header("Location: FrmEdit_listes.php?EIDAPP=" . $EIDAPP);				
}

/* Importer */
if (isset($_POST["BImporter"])) {		
	header("Location: FrmImport_listes.php?EIDAPP=" . $EIDAPP);				
}
...
$resNbre = $data->executeQuery("SELECT COUNT(*) FROM listes;");
$TCount = $util->getLine($resNbre);
$NbreEnreg = 0;
$NbreEnreg = $TCount[0];

// Accès Data
$res = $data->executeQuery("SELECT * FROM listes LIMIT $debut , $fin;");
?>
<!doctype html>
<html lang="en">
<head>
  ...
  <tr>
    <td colspan="7"><input name="BCreer" type="submit" id="BCreer" value="Creer">
      <input name="BSupprimer" type="submit" id="BSupprimer" value="Supprimer">
        ...
        <input name="BGenListe" type="submit" id="BGenListe" value="Generer Liste">
