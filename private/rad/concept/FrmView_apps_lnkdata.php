...
$IDAPPDATLNK = trim(isset($_GET['EIDAPPDATLNK']) ? $_GET['EIDAPPDATLNK'] : '');
...
// Suppression
function supprimerApps_lnkdata($IDAPP,$IDAPPDAT,$IDAPPDATLNK) {
	global $data;
	return $data->executeQuery("DELETE FROM apps_lnkdata WHERE IDAPP = '$IDAPP' AND IDAPPDAT = '$IDAPPDAT' AND IDAPPDATLNK = '$IDAPPDATLNK';");
}
...
if (isset($_POST["BSupprimer"])) {
	if (isset($_POST["ID"])) {
	    ...
	    for ($i = 0; $i < count($id); $i++) {
			$tab = preg_split("/[#]+/", $id[$i]);
			if (count($tab > 0)) {
					supprimerApps_lnkdata(addslashes($tab[0]),addslashes($tab[1]),addslashes($tab[2]));					
			}
		}
	}
}
...
if (isset($_POST["BModifier"])) {
	if (isset($_POST["ID"])) {
		...
		if (count($id > 0) ) {
			...
			header("Location: FrmEdit_apps_lnkdata.php?" . "EIDAPP=" . addslashes($tab[0]) . '&' . "EIDAPPDAT=" . addslashes($tab[1]) . '&' . "EIDAPPDATLNK=" . addslashes($tab[2]));									
		}	
	}
}
...
if (isset($_POST["BCreer"])) {		
    header("Location: FrmEdit_apps_lnkdata.php");				
}
...
// ########################################
// Accès Data
$strSql = "Select * From apps_lnkdata";
...
<form method="post" action="<?php echo $page;?>" name="F5">
...
<table width="100%"  border="1">
  ...
  <tr>
    <td colspan="11"><?php echo $NbreEnreg;?> Apps_lnkdata</td>
  </tr>
  <tr>
    <td width="6%">&nbsp;</td>
		<td width="15%">ID Application</td>
	<td width="15%">ID Template</td>
	<td width="15%">ID Link</td>
	<td width="15%">Nom</td>
	<td width="15%">Libelle</td>
	<td width="15%">Description</td>
	<td width="15%">Lien</td>
	<td width="15%">Parametres</td>
	<td width="15%">Visible</td>
	<td width="15%">No Ordre</td>
</tr>
  ...
  <?php
  	while ($enreg = $util->getLineObject($res)) {  
  		$tab = "  <tr>";
    	$tab .= " <td>";  	
		$tab .= "<input name=\"ID[]\" type=\"checkbox\" id=\"ID[]\" value=\"" . $enreg->IDAPP . "#" . $enreg->IDAPPDAT . "#" . $enreg->IDAPPDATLNK . "\"/>";
		
		$tab .= "</td>";
		$tab .= "<td>" . $enreg->IDAPP . "</td>";
		$tab .= "<td>" . $enreg->IDAPPDAT . "</td>";
		$tab .= "<td>" . $enreg->IDAPPDATLNK . "</td>";
		$tab .= "<td>" . $enreg->NAME . "</td>";
		$tab .= "<td>" . $enreg->LABEL . "</td>";
		$tab .= "<td>" . $enreg->DESCR . "</td>";
		$tab .= "<td>" . $enreg->LINK . "</td>";
		$tab .= "<td>" . $enreg->PARAMS . "</td>";
  ...
  ?>
  <tr>
    <td colspan="11"><input name="BCreer" type="submit" id="BCreer" value="Creer">
...
if ($NbreEnreg > $EnregMax) {
	...
	// Transmission Data
	$var_pri = "&EIDAPP=$IDAPP&EIDAPPDAT=$IDAPPDAT&EIDAPPDATLNK=$IDAPPDATLNK";		
