...
$IDLST = trim(isset($_GET['EIDLST']) ? $_GET['EIDLST'] : '');
$IDLSTLNK = trim(isset($_GET['EIDLSTLNK']) ? $_GET['EIDLSTLNK'] : '');
...
// Suppression
function supprimerLnklist($IDAPP,$IDLST,$IDLSTLNK) {
	global $data;
	return $data->executeQuery("DELETE FROM lnklist WHERE IDAPP = '$IDAPP' AND IDLST = '$IDLST' AND IDLSTLNK = '$IDLSTLNK';");
}
...
if (isset($_POST["BSupprimer"])) {
	if (isset($_POST["ID"])) {
	    ...
	    for ($i = 0; $i < count($id); $i++) {
			$tab = preg_split("/[#]+/", $id[$i]);
			if (count($tab > 0)) {
					supprimerLnklist(addslashes($tab[0]),addslashes($tab[1]),addslashes($tab[2]));					
			}
		}
	}
}
...
if (isset($_POST["BModifier"])) {
	if (isset($_POST["ID"])) {
		...
		if (count($id > 0) ) {
			...
			header("Location: FrmEdit_lnklist.php?" . "EIDAPP=" . addslashes($tab[0]) . '&' . "EIDLST=" . addslashes($tab[1]) . '&' . "EIDLSTLNK=" . addslashes($tab[2]));									
		}	
	}
}
...
if (isset($_POST["BCreer"])) {		
	header("Location: FrmEdit_lnklist.php");				
}
...
<form method="post" action="<?php echo $page;?>" name="FLnklist">
...
<table width="100%"  border="1">
  ...
  <tr>
    <td colspan="11"><?php echo $NbreEnreg;?> Lnklist</td>
  </tr>
  <tr>
    <td width="6%">&nbsp;</td>
		<td width="15%">ID Application</td>
	<td width="15%">ID Liste</td>
  ...
  <?php
  	while ($enreg = $util->getLineObject($res)) {  
  		$tab = "  <tr>";
    	$tab .= " <td>";  	
		$tab .= "<input name=\"ID[]\" type=\"checkbox\" id=\"ID[]\" value=\"" . $enreg->IDAPP . "#" . $enreg->IDLST . "#" . $enreg->IDLSTLNK . "\"/>";
		
		$tab .= "</td>";
		$tab .= "<td>" . $enreg->IDAPP . "</td>";
		$tab .= "<td>" . $enreg->IDLST . "</td>";
		$tab .= "<td>" . $enreg->IDLSTLNK . "</td>";
...
if ($NbreEnreg > $EnregMax) {
	...
	// Transmission Data
	$var_pri = "&EIDAPP=$IDAPP&EIDLST=$IDLST&EIDLSTLNK=$IDLSTLNK";		
