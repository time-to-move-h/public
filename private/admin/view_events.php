...
$IDEVT = trim(isset($_GET['EIDEVT']) ? $_GET['EIDEVT'] : '');
...
// Suppression
function supprimerEvents($IDEVT) {
	global $data;
	return $data->executeQuery("DELETE FROM events WHERE IDEVT = $IDEVT;");
}
...
if (isset($_POST["BSupprimer"])) {
	if (isset($_POST["ID"])) {
	    ...
	    for ($i = 0; $i < count($id); $i++) {
			$tab = preg_split("/[#]+/", $id[$i]);
			if (count($tab > 0)) {
					supprimerEvents(addslashes($tab[0]));					
			}
		}
	}
}
...
if (isset($_POST["BModifier"])) {
	if (isset($_POST["ID"])) {
		...
		if (count($id > 0) ) {
			...
			header("Location: edit_events.php?" . "EIDEVT=" . addslashes($tab[0]));									
		}	
	}
}
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                ...
                <form method="post" action="<?php echo $page;?>" name="FEvents">
                    ...
                    <table width="100%"  border="1">
                      ...
                      <tr>
                        <td colspan="17"><?php echo $NbreEnreg;?> Events</td>
                      </tr>
                      <tr>
                        <td width="6%">&nbsp;</td>
                        	<td width="15%">ID Event</td>
	...
	<td width="15%"></td>
                      ...
                      <?php
                        while ($enreg = $util->getLineObject($res)) {  
                            $tab = "  <tr>";
                            $tab .= " <td>";  	
                            $tab .= "<input name=\"ID[]\" type=\"checkbox\" id=\"ID[]\" value=\"" . $enreg->IDEVT . "\"/>";
                            
                            $tab .= "</td>";
                    		$tab .= "<td>" . $enreg->IDEVT . "</td>";
		$tab .= "<td>" . $enreg->IDGRP . "</td>";
		$tab .= "<td>" . $enreg->EVT_DESC . "</td>";
		$tab .= "<td>" . $enreg->EVT_DESCL . "</td>";
		$tab .= "<td>" . $enreg->EVT_DATBEG . "</td>";
		$tab .= "<td>" . $enreg->EVT_DATEND . "</td>";
		$tab .= "<td>" . $enreg->EVT_FREE . "</td>";
		$tab .= "<td>" . $enreg->EVT_MAXUSE . "</td>";
		$tab .= "<td>" . $enreg->EVT_MULTI . "</td>";
		$tab .= "<td>" . $enreg->EVT_STREET . "</td>";
		$tab .= "<td>" . $enreg->EVT_STREETN . "</td>";
		$tab .= "<td>" . $enreg->EVT_BOX . "</td>";
		$tab .= "<td>" . $enreg->EVT_PCODE . "</td>";
		$tab .= "<td>" . $enreg->EVT_CITY . "</td>";
		$tab .= "<td>" . $enreg->EVT_COUNTRY . "</td>";
		$tab .= "<td>" . $enreg->EVT_DESCRDV . "</td>";
                      ...
                      ?>
                      <tr>
                        <td colspan="17"><input name="BCreer" type="submit" id="BCreer" value="Creer">
