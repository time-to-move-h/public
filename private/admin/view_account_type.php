...
$IDACCTYP = trim(isset($_GET['EIDACCTYP']) ? $_GET['EIDACCTYP'] : '');
...
// Suppression
function supprimerAccount_type($IDACCTYP) {
	global $data;
	return $data->executeQuery("DELETE FROM account_type WHERE IDACCTYP = $IDACCTYP;");
}
...
if (isset($_POST["BSupprimer"])) {
	if (isset($_POST["ID"])) {
	    ...
	    for ($i = 0; $i < count($id); $i++) {
			$tab = preg_split("/[#]+/", $id[$i]);
			if (count($tab > 0)) {
					supprimerAccount_type(addslashes($tab[0]));					
			}
		}
	}
}
...
if (isset($_POST["BModifier"])) {
	if (isset($_POST["ID"])) {
		...
		if (count($id > 0) ) {
			...
			header("Location: edit_account_type.php?" . "EIDACCTYP=" . addslashes($tab[0]));									
		}	
	}
}
...
if (isset($_POST["BCreer"])) {		
	header("Location: edit_account_type.php");				
}
...
// ########################################
// Accès Data
$strSql = "Select * From account_type";
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                ...
                <form method="post" action="<?php echo $page;?>" name="FAccount_type">
                    ...
                    <table width="100%"  border="1">
                      ...
                      <tr>
                        <td colspan="4"><?php echo $NbreEnreg;?> Account_type</td>
                      </tr>
                      <tr>
                        <td width="6%">&nbsp;</td>
                        	<td width="15%">ID</td>
	...
	<td width="15%">Active</td>
                      ...
                      <?php
                        while ($enreg = $util->getLineObject($res)) {  
                            $tab = "  <tr>";
                            $tab .= " <td>";  	
                            $tab .= "<input name=\"ID[]\" type=\"checkbox\" id=\"ID[]\" value=\"" . $enreg->IDACCTYP . "\"/>";
                            
                            $tab .= "</td>";
                    		$tab .= "<td>" . $enreg->IDACCTYP . "</td>";
		$tab .= "<td>" . $enreg->ACCTYP_DESC . "</td>";
		$tab .= "<td>" . $enreg->ACCTYP_ACTIVE . "</td>";
