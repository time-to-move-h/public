...
// Suppression
function supprimerEvents_list($IDEVT,$IDUSER) {
	global $data;
	return $data->executeQuery("DELETE FROM events_list WHERE IDEVT = $IDEVT AND IDUSER = $IDUSER;");
}
...
if (isset($_POST["BSupprimer"])) {
	if (isset($_POST["ID"])) {
	    ...
	    for ($i = 0; $i < count($id); $i++) {
			$tab = preg_split("/[#]+/", $id[$i]);
			if (count($tab > 0)) {
					supprimerEvents_list(addslashes($tab[0]),addslashes($tab[1]));					
			}
		}
	}
}
...
if (isset($_POST["BModifier"])) {
	if (isset($_POST["ID"])) {
		...
		if (count($id > 0) ) {
			...
			header("Location: edit_events_list.php?" . "EIDEVT=" . addslashes($tab[0]) . '&' . "EIDUSER=" . addslashes($tab[1]));									
		}	
	}
}
...
if (isset($_POST["BCreer"])) {		
	header("Location: edit_events_list.php");				
}
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                ...
                <form method="post" action="<?php echo $page;?>" name="FEvents_list">
                    ...
                    <table width="100%"  border="1">
                      ...
                      <tr>
                        <td colspan="7"><?php echo $NbreEnreg;?> Events_list</td>
                      </tr>
                      <tr>
                        <td width="6%">&nbsp;</td>
                        	<td width="15%">ID Event</td>
	<td width="15%">ID User</td>
	<td width="15%">Friends Number</td>
	<td width="15%">Waiting List</td>
	<td width="15%">Begin Date</td>
	<td width="15%">Organizer</td>
                      ...
                      <?php
                        while ($enreg = $util->getLineObject($res)) {  
                            $tab = "  <tr>";
                            $tab .= " <td>";  	
                            $tab .= "<input name=\"ID[]\" type=\"checkbox\" id=\"ID[]\" value=\"" . $enreg->IDEVT . "#" . $enreg->IDUSER . "\"/>";
                            
                            $tab .= "</td>";
                    		$tab .= "<td>" . $enreg->IDEVT . "</td>";
		$tab .= "<td>" . $enreg->IDUSER . "</td>";
		$tab .= "<td>" . $enreg->EVTLST_N_FRIEND . "</td>";
		$tab .= "<td>" . $enreg->EVTLST_WAIT . "</td>";
		$tab .= "<td>" . $enreg->EVTLST_DATBEG . "</td>";
		$tab .= "<td>" . $enreg->EVTLST_ORG . "</td>";
                    ...
                    if ($NbreEnreg > $EnregMax) {
                        ...
                        // Transmission Data
                        $var_pri = "&EIDEVT=$IDEVT&EIDUSER=$IDUSER";		
