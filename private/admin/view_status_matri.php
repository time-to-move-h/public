...
$IDSTMA = trim(isset($_GET['EIDSTMA']) ? $_GET['EIDSTMA'] : '');
...
// Suppression
function supprimerStatus_matri($IDSTMA) {
	global $data;
	return $data->executeQuery("DELETE FROM status_matri WHERE IDSTMA = $IDSTMA;");
}
...
if (isset($_POST["BSupprimer"])) {
	if (isset($_POST["ID"])) {
	    ...
	    for ($i = 0; $i < count($id); $i++) {
			$tab = preg_split("/[#]+/", $id[$i]);
			if (count($tab > 0)) {
					supprimerStatus_matri(addslashes($tab[0]));					
			}
		}
	}
}
...
if (isset($_POST["BModifier"])) {
	if (isset($_POST["ID"])) {
		...
		if (count($id > 0) ) {
			...
			header("Location: edit_status_matri.php?" . "EIDSTMA=" . addslashes($tab[0]));									
		}	
	}
}
...
if (isset($_POST["BCreer"])) {		
	header("Location: edit_status_matri.php");				
}
...
// ########################################
// Accès Data
$strSql = "Select * From status_matri";
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                ...
                <form method="post" action="<?php echo $page;?>" name="FStatus_matri">
                    ...
                    <table width="100%"  border="1">
                      ...
                      <tr>
                        <td colspan="3"><?php echo $NbreEnreg;?> Status_matri</td>
                      </tr>
                      <tr>
                        <td width="6%">&nbsp;</td>
                        	<td width="15%">ID</td>
	<td width="15%">Description</td>
                      ...
                      <?php
                        while ($enreg = $util->getLineObject($res)) {  
                            $tab = "  <tr>";
                            $tab .= " <td>";  	
                            $tab .= "<input name=\"ID[]\" type=\"checkbox\" id=\"ID[]\" value=\"" . $enreg->IDSTMA . "\"/>";
                            
                            $tab .= "</td>";
                    		$tab .= "<td>" . $enreg->IDSTMA . "</td>";
		$tab .= "<td>" . $enreg->STMA_DESC . "</td>";
                      ...
                      ?>
                      <tr>
                        <td colspan="3"><input name="BCreer" type="submit" id="BCreer" value="Creer">
