...
$IDCOUNTRY = trim(isset($_GET['EIDCOUNTRY']) ? $_GET['EIDCOUNTRY'] : '');
...
// Suppression
function supprimerCountry($IDCOUNTRY) {
	global $data;
	return $data->executeQuery("DELETE FROM country WHERE IDCOUNTRY = $IDCOUNTRY;");
}
...
if (isset($_POST["BSupprimer"])) {
	if (isset($_POST["ID"])) {
	    ...
	    for ($i = 0; $i < count($id); $i++) {
			$tab = preg_split("/[#]+/", $id[$i]);
			if (count($tab > 0)) {
					supprimerCountry(addslashes($tab[0]));					
			}
		}
	}
}
...
if (isset($_POST["BModifier"])) {
	if (isset($_POST["ID"])) {
		...
		if (count($id > 0) ) {
			...
			header("Location: edit_country.php?" . "EIDCOUNTRY=" . addslashes($tab[0]));									
		}	
	}
}
...
if (isset($_POST["BCreer"])) {		
	header("Location: edit_country.php");				
}
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                ...
                <form method="post" action="<?php echo $page;?>" name="FCountry">
                    ...
                    <table width="100%"  border="1">
                      ...
                      <tr>
                        <td colspan="7"><?php echo $NbreEnreg;?> Country</td>
                      </tr>
                      <tr>
                        <td width="6%">&nbsp;</td>
                        	<td width="15%">identifiant pays</td>
	<td width="15%">description</td>
	<td width="15%">Country FR</td>
	<td width="15%">Country ES</td>
	<td width="15%">Country NL</td>
	<td width="15%">Active</td>
                      ...
                      <?php
                        while ($enreg = $util->getLineObject($res)) {  
                            $tab = "  <tr>";
                            $tab .= " <td>";  	
                            $tab .= "<input name=\"ID[]\" type=\"checkbox\" id=\"ID[]\" value=\"" . $enreg->IDCOUNTRY . "\"/>";
                            
                            $tab .= "</td>";
                    		$tab .= "<td>" . $enreg->IDCOUNTRY . "</td>";
		$tab .= "<td>" . $enreg->COUNT_DESC_EN . "</td>";
		$tab .= "<td>" . $enreg->COUNT_DESC_FR . "</td>";
		$tab .= "<td>" . $enreg->COUNT_DESC_ES . "</td>";
		$tab .= "<td>" . $enreg->COUNT_DESC_NL . "</td>";
