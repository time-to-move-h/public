...
$IDSTCON = trim(isset($_GET['EIDSTCON']) ? $_GET['EIDSTCON'] : '');
...
// Suppression
function supprimerStatus_con($IDSTCON) {
	global $data;
	return $data->executeQuery("DELETE FROM status_con WHERE IDSTCON = $IDSTCON;");
}
...
if (isset($_POST["BSupprimer"])) {
	if (isset($_POST["ID"])) {
	    ...
	    for ($i = 0; $i < count($id); $i++) {
			$tab = preg_split("/[#]+/", $id[$i]);
			if (count($tab > 0)) {
					supprimerStatus_con(addslashes($tab[0]));					
			}
		}
	}
}
...
if (isset($_POST["BModifier"])) {
	if (isset($_POST["ID"])) {
		...
		if (count($id > 0) ) {
			...
			header("Location: edit_status_con.php?" . "EIDSTCON=" . addslashes($tab[0]));									
		}	
	}
}
...
if (isset($_POST["BCreer"])) {		
	header("Location: edit_status_con.php");				
}
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                ...
                <form method="post" action="<?php echo $page;?>" name="FStatus_con">
                    ...
                    <table width="100%"  border="1">
                      ...
                      <tr>
                        <td colspan="4"><?php echo $NbreEnreg;?> Status_con</td>
                      </tr>
                      <tr>
                        <td width="6%">&nbsp;</td>
                        	<td width="15%">ID</td>
	...
	<td width="15%">Active</td>
                      ...
                      <?php
                        while ($enreg = $util->getLineObject($res)) {  
                            $tab = "  <tr>";
                            $tab .= " <td>";  	
                            $tab .= "<input name=\"ID[]\" type=\"checkbox\" id=\"ID[]\" value=\"" . $enreg->IDSTCON . "\"/>";
                            
                            $tab .= "</td>";
                    		$tab .= "<td>" . $enreg->IDSTCON . "</td>";
		$tab .= "<td>" . $enreg->STCON_DESC . "</td>";
		$tab .= "<td>" . $enreg->STCON_ACTIVE . "</td>";
