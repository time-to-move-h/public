...
(function($, window, document, undefined) {
    $.fn.socialfeed = function(_options) {
        ...
        var defaults = {
            ...
            template: '/tpl/social_template', // a path to the template file
        ...
        var options = $.extend(defaults, _options),
            container = $(this),
            template,
            social_networks = ['facebook', 'instagram', 'vk', 'google', 'blogspot', 'twitter', 'pinterest', 'rss', 'moviao'],
...
//                            break;
                        case '@':
                            var username = account.substr(1);
                            request_url = Feed.moviao.api + '?uid=' + username + '&maxResults=' + options.moviao.limit; //'&key=' + options.moviao.access_token + 
                            Utility.get_request(request_url, Feed.moviao.utility.getPosts);
                            break;
                        default:
                    }
                },
                utility: {
                    getPosts: function(json) {                         
                        if (json.items) {
                            $.each(json.items, function(i) {
                                var post = new SocialFeedPost('moviao', Feed.moviao.utility.unifyPostData(json.items[i]));
                                post.render();
                            });
                        }
                    },
                    unifyPostData: function(element) {
                        ...
                        post.description = '';
                        post.dt_create = moment(element.published);
                        post.author_link = 'url actor'; //element.actor.url;
                        post.author_picture = element.actor.imageurl;
                        post.author_name = element.actor.displayName;
