...
//        echoBotUser.Status = 1 /* Online */;
        ...
        getContactsPM(this, function(self, model) {                             
            if (model != null && model.result === true) { 
                for (i = 0; i < model.data.length; i++) { 
                    //console.log(model.data[i]);
                    var ndisp = model.data[i]['NDISP'];
                    var uuid = model.data[i]['UUID'];
                    var img = model.data[i]['PICTURE'];
                    //chat.loadPeople(ndisp,img,uuid,'');  
                    
                    console.log(ndisp);
                                        
                    var contactUser = new ChatUserInfo();
                    contactUser.Id = (i+1);
                    contactUser.RoomId = DemoAdapterConstants.DEFAULT_ROOM_ID;
                    contactUser.Name = ndisp;
                    contactUser.Email = "echobot1984@gmail.com";
                    contactUser.ProfilePictureUrl = img;
                    contactUser.Status = 1 /* Online */;                    
                    self.users.push(contactUser);                    
                } 
                
                
                    self.users.push(myUser);
                    //this.users.push(echoBotUser);
                    // configuring rooms
                    var defaultRoom = new ChatRoomInfo();
                    defaultRoom.Id = 1;
                    defaultRoom.Name = "Default Room";
                    defaultRoom.UsersOnline = self.users.length;
                    self.rooms = new Array();
                    self.rooms.push(defaultRoom);
                    // configuring client to return every event to me
                    self.clientAdapter.onMessagesChanged(function (message) { return function () {        }; });
    ...
    DemoServerAdapter.prototype.getUserInfo = function (userId, done) {
        ...
        alert("commence a verifier users");
